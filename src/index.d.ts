...
declare namespace Charm {
	...
	type SyncPatch<T> = {
		readonly [P in keyof T]?:
			| (T[P] extends object ? SyncPatch<T[P]> : T[P])[]
			| (T[P] extends undefined ? None : never)[];
	};

	type DeriveStates<T> = { [P in keyof T]: T[P] extends Atom<infer S> ? S : T[P] };

	/**
	 * A payload that can be sent from the server to the client to synchronize
	 * state between the two.
	 */
	type SyncPayload<T> = { type: "init"; data: DeriveStates<T> } | { type: "patch"; data: SyncPatch<DeriveStates<T>> };
